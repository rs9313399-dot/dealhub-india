...
/** Slugs that have dedicated Next.js routes at /{slug} (not hash-only). */
export const directCategorySlugs = categories.map((c) => c.slug);

export function isDirectCategorySlug(slug: string): boolean {
  return directCategorySlugs.includes(slug);
}
